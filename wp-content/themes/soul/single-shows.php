	  ...
	  <input class="id" type="hidden" name="id" value="<?php echo $current_user->ID;  ?>" >
		...
		?>
                ...
                <div class="other_tv_show">
                    <h2>OTHER TOP Tv shows</h2>
                    <ul>
                    
    <?php 
				global $post;
				query_posts('post_type=shows&showposts=4&order=desc&meta_key =_is_ns_featured_post&meta_value= yes'); 
				while (have_posts()) : the_post();
				?>
             <li>
		   ...
		   <?php 
 $args = array('post_type' => 'shows','posts_per_page'=>-1,'order' => 'DESC','post_status'=> 'publish');
